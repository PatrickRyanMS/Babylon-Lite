// Scene 56 — Billboard Yaw-Locked / Cylindrical (Family 3).
//
...
// Comparing this scene's golden against scene 55's exposes the difference:
// here the top edges remain world-vertical, while in scene 55 they tilt with
// the camera.
//
// Reference path: BJS has no native yaw-locked SpriteManager, so the BJS
// reference (`bjs/scene56.ts`) builds its own quads via textured planes with
