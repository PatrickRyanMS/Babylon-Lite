/**
 * Scene 52 — Sprites Animated Parity Test (Family 1)
 *
...
const REFERENCE_DIR = path.resolve(__dirname, "../../../reference/scene52-sprites-animated");
const GOLDEN_REF = path.join(REFERENCE_DIR, "babylon-ref-golden.png");

test("Scene 52 — Sprites Animated matches canvas2D reference at seekTime=1.0s", async ({ page }, testInfo) => {
    const browser = page.context().browser()!;
    await captureGolden(browser, { sceneId: 33, seekTime: SEEK_TIME });

    await page.goto(`/scene52.html?seekTime=${SEEK_TIME}`);
