...
| `new ShadowGenerator(size, light)` | `await createShadowGenerator(engine, light, casters, opts)` |
...
// ✅ Babylon Lite
light.shadowGenerator = await createShadowGenerator(engine, light, [mesh], {
    mapSize: 1024,
    depthScale: 50,
    blurScale: 2,
});
