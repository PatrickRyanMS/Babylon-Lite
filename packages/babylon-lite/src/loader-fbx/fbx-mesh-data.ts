...
export interface FbxMeshGpuData {
    ...
    /** Per-vertex colors `[r,g,b,a, …]` (tight RGBA matching the engine's float32x4 vertex-color
     *  layout; alpha forced to 1.0 — BJS forces vertex alpha to 1.0), or null when absent. */
...
export function buildFbxMeshData(geom: FBXGeometryData, geomT: FBXVector3, geomR: FBXVector3, geomS: FBXVector3): FbxMeshGpuData {
    ...
    // Colors are F32 RGBA from the interpreter — repack to a tight F32 RGBA buffer matching the
    // engine-wide float32x4 vertex-color layout, forcing alpha to 1.0 (BJS forces vertex alpha to
    // 1.0; the Standard pipeline uses the rgb to modulate base color).
    let colors: Float32Array | null = null;
    if (geom.colors) {
        const vcount = (geom.colors.length / 4) | 0;
        colors = new F32(vcount * 4);
        for (let v = 0; v < vcount; v++) {
            colors[v * 4] = geom.colors[v * 4]!;
            colors[v * 4 + 1] = geom.colors[v * 4 + 1]!;
            colors[v * 4 + 2] = geom.colors[v * 4 + 2]!;
            colors[v * 4 + 3] = 1.0; // BJS forces vertex alpha to 1.0
        }
    }
