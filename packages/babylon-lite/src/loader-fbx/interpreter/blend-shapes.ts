/**
 * FBX blend-shape (morph target) extraction (PURE, zero engine/GPU dependency).
 *
 * Faithful port of the Babylon.js FBX loader's `blendShapes.ts`: walks the
 * resolved object/connection graph to collect every `BlendShape` deformer, its
 * `BlendShapeChannel` children, and the sparse `Shape` geometries that hold the
 * per-control-point position (and optional normal) deltas.
 *
 * Shape `Vertices` are SPARSE delta offsets from the base geometry, and
 * `Indexes` lists which control points each delta belongs to. Turning those
 * sparse deltas into dense per-output-vertex arrays (and baking the geometric
 * delta transform) lives in `../fbx-morph-data.ts`; this module only reads data.
 */

import type { FBXNode } from "../types/fbx-types.js";
import { cleanFBXName, findChildByName, getPropertyValue } from "../types/fbx-types.js";
...
export interface FBXShapeData {
    /** Sparse control-point indices affected by this shape. */
    indices: Uint32Array;
    /** Position deltas for the affected control points `[x,y,z, …]`. */
    vertices: Float64Array;
    /** Normal deltas for the affected control points `[x,y,z, …]`, or null. */
    normals: Float64Array | null;
}

/** Recoverable blend-shape import issue. */
...
export function extractBlendShapes(objectMap: FBXObjectMap): FBXBlendShapeData[] {
    ...
    for (const [id, node] of Array.from(objectMap.objects)) {
        if (node.name === "Deformer" && getPropertyValue<string>(node, 2) === "BlendShape") {
            const bs = extractBlendShape(id, objectMap);
            if (bs) {
                blendShapes.push(bs);
            }
        }
...
function extractBlendShape(deformerId: number, objectMap: FBXObjectMap): FBXBlendShapeData | null {
...
function extractShape(shapeNode: FBXNode): FBXShapeData | null {
    // Shape has: Indexes (sparse control-point indices), Vertices (delta offsets
    // from base), Normals (optional delta).
