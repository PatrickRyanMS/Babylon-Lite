/**
 * FBX skin-deformer extraction — PURE interpreter pass.
 *
 * Ported faithfully from the Babylon.js FBX loader (`FBX/interpreter/skeleton.ts`):
 * walks the connection graph, collects every `Deformer`/`Skin` together with its
 * `Cluster` children, resolves the bone (Model) hierarchy, and reads the per
 * control-point bone indices + weights. The output is plain data — no engine, no
 * GPU. The skinning weight expansion to per-output-vertex buffers and the GPU
 * skeleton are built later by `fbx-skeleton-data.ts` / `fbx-skeleton-build.ts`.
 *
 * Cluster matrices (`Transform` = mesh bind global, `TransformLink` = bone bind
 * global) are preserved as `Float64Array(16)` flat values. A BJS row-major matrix
 * and a Lite column-major `Mat4` share identical flat storage for the same
 * transform (see `fbx-mat4.ts`), so these arrays are directly usable as Lite
 * matrices with no transpose.
 */

import type { FBXNode } from "../types/fbx-types.js";
import { findChildByName, getPropertyValue, cleanFBXName } from "../types/fbx-types.js";
...
export interface FBXSkinDiagnostic {
    ...
    message: string;
    /** Bone Model node ID associated with the diagnostic, if applicable. */
    boneModelId?: number;
    /** Bone name associated with the diagnostic, if applicable. */
    boneName?: string;
    /** Cluster mode associated with the diagnostic, if applicable. */
...
export interface FBXBoneData {
    ...
    clusterMode: FBXClusterMode;
    /** Bind pose transform matrix (cluster Transform, mesh bind global, 4x4 flat). */
    bindPoseMatrix: Float64Array | null;
    /** Bone's world transform at bind time (cluster TransformLink, 4x4 flat). */
    transformLinkMatrix: Float64Array | null;
    /** Associate model world transform at bind time (cluster TransformAssociateModel, 4x4 flat). */
    transformAssociateModelMatrix: Float64Array | null;
    /** Model's absolute matrix from the FBX BindPose, when present (4x4 flat). */
...
export interface FBXSkinData {
    ...
    geometryId: number;
    /** Mesh model world matrix from the FBX BindPose, when present (4x4 flat). */
    meshBindPoseMatrix: Float64Array | null;
    /** Bones in this skeleton. */
    bones: FBXBoneData[];
    /** Per control-point bone indices, sorted by descending weight and capped for skinning. */
    boneIndices: number[][];
    /** Per control-point bone weights, matching boneIndices. */
    boneWeights: number[][];
...
/**
 * Extract all skin deformers from the FBX scene. Returns skin data including
 * bone hierarchy and per control-point vertex weights.
 */
export function extractSkins(objectMap: FBXObjectMap): FBXSkinData[] {
    ...
    for (const [id, node] of Array.from(objectMap.objects)) {
        if (node.name === "Deformer" && getPropertyValue<string>(node, 2) === "Skin") {
            const skin = extractSkin(id, objectMap);
            if (skin) {
                skins.push(skin);
            }
        }
...
function extractSkin(skinId: number, objectMap: FBXObjectMap): FBXSkinData | null {
    // Find the geometry this skin is attached to. The Skin is a child of the
    // geometry in the FBX connection graph.
    ...
    // For each cluster, find the connected bone Model. Connection graph:
    // BoneModel → Cluster (bone is a child of the cluster).
    ...
    // Extract per control-point weights from clusters.
...
function buildBoneHierarchy(
    ...
    // BFS to build the ordered list (parents before children).
...
/** True when a Model node is a skeleton joint (Root or LimbNode sub-type). */
...
/** Read the full FBX node transform components from a Model node's Properties70. */
...
function createBoneDiagnostics(
    ...
    if (clusterMode === "Additive" || clusterMode === "TotalOne") {
        diagnostics.push({
            ...
            message: `Cluster mode '${clusterMode}' is preserved but not applied by linear blend skinning.`,
            boneModelId: modelId,
            boneName,
            clusterMode,
        });
...
/**
 * Extract per control-point bone indices and weights from cluster data. Returns
 * arrays indexed by control point index, sorted by descending weight and capped
 * to {@link MAX_BONE_INFLUENCES}, with weights normalized to sum to 1.
 */
function extractVertexWeights(bones: FBXBoneData[], boneModelMap: Map<number, { clusterId: number; clusterNode: FBXNode }>): { boneIndices: number[][]; boneWeights: number[][] } {
    // We need the max control-point index to size the arrays.
    ...
    // Sort by weight descending and cap to the primary + extra influence buffers.
    for (let i = 0; i < vertexCount; i++) {
