/**
 * FBX animation interpreter — pure, faithful port of the Babylon.js FBX loader's
 * `interpreter/animation.ts`. It resolves the AnimationStack → AnimationLayer →
 * AnimationCurveNode → AnimationCurve graph into plain per-axis keyframe curves.
 *
 * This module is DATA-ONLY: it reads the resolved {@link FBXObjectMap} and emits
 * curve structures. It never touches Lite scene nodes, matrices, or the runtime
 * animation system — the build step (`fbx-animation-build.ts`) consumes these
 * structures and produces Lite {@link AnimationSampler}s/{@link AnimationChannel}s.
 *
 * FBX time is stored in ticks of `1 / 46186158000` second; every keyframe time
 * here is already converted to seconds. In Lite the parsers store `KeyTime`
 * (int64[]) as a `Float64Array` and `KeyValueFloat` (float32[]) as a
 * `Float32Array`, so the port reads those typed arrays directly (no bigint).
 */
...
/** An animation curve node (T/R/S for one model, or DeformPercent for a blend shape). */
export interface FBXCurveNodeData {
    /** Property type: "T" (translation), "R" (rotation), "S" (scale), "DeformPercent". */
    type: string;
    /** Target model (or blend-shape channel) ID. */
...
export interface FBXAnimationStackData {
    ...
    /** Duration in seconds. */
    duration: number;
    /** Per-model curve nodes (flattened from all layers for backward compat). */
    curveNodes: FBXCurveNodeData[];
...
function extractCurveNode(curveNodeId: number, curveNodeNode: FBXNode, objectMap: FBXObjectMap): FBXCurveNodeData | null {
    ...
    if (typeName === "T" || typeName === "R" || typeName === "S") {
        ...
        return { type: typeName, targetModelId, curves };
    }
    ...
    if (typeName === "DeformPercent") {
        ...
        return { type: "DeformPercent", targetModelId: targetId, curves };
    }
...
/**
 * Find the Model that an AnimationCurveNode targets. The CurveNode connects to
 * the Model via an OP connection carrying the property name (e.g. "Lcl Translation").
 */
...
function findCurveNodeBlendShapeTarget(curveNodeId: number, objectMap: FBXObjectMap): number | null {
    for (const conn of objectMap.connections) {
        if (conn.childId === curveNodeId && conn.type === "OP") {
            const parentNode = objectMap.objects.get(conn.parentId);
            if (parentNode && parentNode.name === "Deformer" && getPropertyValue<string>(parentNode, 2) === "BlendShapeChannel") {
                return conn.parentId;
            }
        }
    }
...
/**
 * Extract AnimationCurves connected to a CurveNode. Each curve connects via OP
 * with channel "d|X", "d|Y", or "d|Z" (some exporters use OO and channel order).
 */
...
function extractKeyframes(curveNode: FBXNode): FBXKeyframe[] {
    ...
    const keyTimes = toFloat64Array(keyTimeNode.properties[0]?.value);
...
/** Sample a single FBX curve at `time` (seconds). Returns null when the curve is empty. */
