/** Fog WGSL helper — split out of `wgsl-helpers.ts` so the always-present fog block
 *  in the Standard/PBR templates does not drag the bump (`perturbNormal`) and dither
 *  helpers (only used by lazily-imported bump/background fragments) into the eager
 *  material chunk. Scenes without those features then never fetch `wgsl-helpers`. */
